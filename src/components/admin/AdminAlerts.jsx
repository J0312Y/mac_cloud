    ...
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex gap-1 flex-wrap">
          {["all","unack","critical","warning","info"].map(f=>(<button key={f} onClick={()=>setFilter(f)} className={`px-2.5 py-1 rounded-lg text-[9px] capitalize font-bold transition-colors ${filter===f?"accent-bg-dyn/25 accent-text-dyn border accent-bd-dyn":"text-slate-500 hover:text-slate-300"}`}>{f}</button>))}
        </div>
      ...
      <div className="space-y-2">
        {filtered.map(a=>(
          <div key={a.id} className={`bg-[#13111f] border rounded-xl px-4 py-3 flex items-start gap-3 transition-opacity ${a.ack?"opacity-50":"border-white/[0.06]"} ${a.type==="critical"&&!a.ack?"accent-bd-dyn":""}`}>
            ...
            {!a.ack&&<button onClick={()=>ack(a.id)} className="text-[10px] accent-text-dyn font-bold hover:accent-text-dyn transition-colors flex-shrink-0">Ack</button>}
