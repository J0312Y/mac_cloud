  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[["Total Backups",bups.length,"text-slate-300"],["Successful",bups.filter(b=>b.status==="success").length,"text-emerald-400"],["Failed",bups.filter(b=>b.status==="failed").length,"accent-text-dyn"],["Total Size","113.7 GB","accent-text"]].map(([l,v,c])=>(
      ...
      <C>
        <CH title="Backup History" sub="Silent mode — users never notified" action={<button onClick={trigger} className="flex items-center gap-1.5 px-3 py-1.5 accent-bg-dyn/20 border accent-bd-dyn accent-text-dyn text-[10px] font-bold rounded-lg hover:accent-bg-dyn/30 transition-colors"><Icon name="database" size={11}/>Trigger Backup</button>}/>
