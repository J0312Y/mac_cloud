      ...
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '80px 56px', position: 'relative' }}>
        <div style={{ maxWidth: 380, width: '100%' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginBottom: 24 }}>
            {[['🐙','Continue with GitHub'],['🦊','Continue with GitLab']].map(([ico, lbl]) => (
              <button key={lbl} onClick={() => toast('OAuth coming soon — use email below', 'info')}
                className="btn-ghost" style={{ width: '100%', justifyContent: 'center', padding: '11px', fontSize: 14 }}>
                <span>{ico}</span> {lbl}
              </button>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                ...
                <a href="#" style={{ fontSize: 12, color: 'var(--cyan)', textDecoration: 'none', fontWeight: 500 }}>Forgot?</a>
              </div>
